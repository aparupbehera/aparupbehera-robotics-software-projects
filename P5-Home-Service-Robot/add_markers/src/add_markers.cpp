...
void trackOdom(const nav_msgs::Odometry::ConstPtr& msg){

	float robot_pose_x = msg->pose.pose.position.x;
	float robot_pose_y = msg->pose.pose.position.y;
	float robot_pose_z = msg->pose.pose.position.z;
	...
	if(pick_up && !drop_off){
		dropoff_dis = sqrt(pow((dropoff_x - robot_pose_x), 2) + pow((dropoff_y - robot_pose_y), 2));	
                ROS_INFO("robot_pose_x = %f", robot_pose_x);
		ROS_INFO("robot_pose_y = %f", robot_pose_y);
		ROS_INFO("robot_pose_z = %f", robot_pose_z);
...
int main( int argc, char** argv ){
	ros::init(argc, argv, "add_markers");
  	...
  	ros::Subscriber odometry_sub = n.subscribe("/odom", 1000, trackOdom);
